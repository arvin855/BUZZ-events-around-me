        ...
        if(exif_imagetype($_FILES["fileUpload"]["tmp_name"])){ // Check if the file is actually an image.
            if(isset($_POST['profile'])){
                ...
                if(move_uploaded_file($_FILES["fileUpload"]["tmp_name"], $filepath)){
                    $sql5 = "INSERT INTO posts (post_type, post_caption, post_time, post_by)
                            VALUES ('profile', '" . $row['fname'] . " " . $row['lname'] . " has changed  profile picture.', NOW(), {$_SESSION['user_id']})";
                ...
                if($success = 1)
                    header("location:profile_samp.php?id=" . $_SESSION['user_id']);
            }
